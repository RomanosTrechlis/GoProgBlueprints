...
func TestParseCost(t *testing.T) {
	is := is.New(t)
	is.Equal(int(meander.Cost1), meander.ParseCost("$"))
	is.Equal(int(meander.Cost2), meander.ParseCost("$$"))
	is.Equal(int(meander.Cost3), meander.ParseCost("$$$"))
	is.Equal(int(meander.Cost4), meander.ParseCost("$$$$"))
	is.Equal(int(meander.Cost5), meander.ParseCost("$$$$$"))
}

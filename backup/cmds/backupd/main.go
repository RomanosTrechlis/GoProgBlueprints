...
import (
	...
	"github.com/RomanosTrechlis/GoProgBlueprints/backup"
...
func main() {
	...
	col.ForEach(func(_ int, data []byte) bool {
		if err := json.Unmarshal(data, &pathA); err != nil {
			fatalErr = err
			return true
		}
		m.Paths[pathA.Path] = pathA.Hash
	...
	for {
		...
		case <-signalChan:
			...
			fmt.Print("Saving paths to db:\n")
			for p, hash := range m.Paths {
				var pathB path
				pathB.Path = p
				pathB.Hash = hash
				if err := col.InsertJSON(pathB); err != nil {
					fatalErr = err
					return
				}
				fmt.Printf("\t+ %s\n", pathB)
